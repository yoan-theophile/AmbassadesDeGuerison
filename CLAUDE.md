...
- **Photo de profil visiteur** : bucket Storage privé dédié `visitor-photos` (distinct de `ambassador-photos`), compressée en WebP, validée par magic bytes via `sharp()` (pas seulement le `Content-Type` déclaré par le client) avec garde anti decompression-bomb (dimensions max 8000px). Un échec de traitement ne bloque jamais la création de compte (`photo_error: true` dans la réponse). Visible par l'hôte via `POST /api/dashboard/contact-photos` (signed URL 15 min, vérifie l'ownership de la demande de visite — ne révèle jamais l'email/téléphone du visiteur par cette route).
