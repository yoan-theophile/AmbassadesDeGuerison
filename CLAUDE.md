...
- `POST /api/visit-requests` appelle `createOrUpdateVisitorProfile()` en best-effort (`.catch(() => {})`, ne bloque jamais la demande) : crée un compte Supabase Auth (`user_metadata.role = 'visitor'`) si l'email est neuf (fallback `listUsers` si déjà enregistré), puis upsert `visitor_profiles (email, phone)`.
...
- `/mon-espace` (`app/mon-espace/page.tsx`) : espace minimal, **pas un dashboard complet** — email (lecture seule), téléphone éditable (`PATCH /api/visitor/profile`), déconnexion. Phase 5 (espace visiteur enrichi) reste hors scope, pas de besoin identifié par David.
- `ContactForm` et `VisitRequestForm` préremplissent email/téléphone si une session visiteur existe (`GET /api/visitor/profile`), et affichent un lien "Déjà venu ? Se connecter" vers `/auth`.
- Téléphone visiteur **obligatoire** (contrainte `contact_requests.visitor_phone NOT NULL` + validation `isValidPhoneNumber` côté client) — permet à l'hôte d'appeler directement en cas de besoin.
