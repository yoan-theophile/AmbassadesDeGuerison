...
export async function PATCH(req: NextRequest) {
  ...
  const { city, lat, lng, country, address_private, lat_precise, lng_precise, consignes, phone, quartier, presentation_message, is_women_only } = body;

  const updates: Record<string, unknown> = {};

  if (address_private !== undefined) updates.address_private = address_private;
  if (lat_precise !== undefined && lng_precise !== undefined) {
    const latP = Number(lat_precise);
    const lngP = Number(lng_precise);
    if (Number.isNaN(latP) || latP < -90 || latP > 90 || Number.isNaN(lngP) || lngP < -180 || lngP > 180) {
      return NextResponse.json({ error: 'Coordonnées précises invalides.' }, { status: 400 });
    }
    updates.lat_precise = latP;
    updates.lng_precise = lngP;
  }
